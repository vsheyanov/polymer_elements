...
main() async {
    ...
    group('basic', () {
        ...
        test('empty required input shows error', () {
            ...
            expect(error.getComputedStyle().display, isNot('none'), reason: 'error should not be display:none');
        });
        ...
        test('invalid input shows error message after manual validation', () {
            ...
            expect(error.getComputedStyle().display, equals('none'), reason: 'error should be display:none');
            input.validate();
            expect(error.getComputedStyle().display, isNot('none'), reason: 'error should not be display:none');
        });
        ...
        test('caret position is preserved', () {
            ...
            expect(ironInput.selectionStart, equals(2), reason: 'selectionStart should be preserved');
            expect(ironInput.selectionEnd, equals(2), reason: 'selectionEnd should be preserved');
        });
    });
    ...
    group('a11y', () {
        test('has aria-labelledby', () {
            ...
            expect(input.inputElement.getAttribute('aria-labelledby'), input.querySelector('label').id, reason: 'aria-labelledby points to the label');
        });
